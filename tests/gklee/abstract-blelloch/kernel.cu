...
__global__ void blelloch(rtype *len, rtype *out, unsigned *error) {
  ...
  for (unsigned d = 1; d < N; d *= 2) {
    ...
    if (t < d) {
      unsigned ai = offset * (2 * t + 1) - 1;
      unsigned bi = offset * (2 * t + 2) - 1;
      if ( !((result[bi].x <= result[bi].y) &&
             (                result[bi].y == result[ai].x) &&
             (                                result[ai].x <= result[ai].y)) ) {
        *error = 1;
      }
...
int main(int argc, char **argv) {
...
#endif
  blelloch<<<1,(N/2)>>>(d_in, d_out, d_error);
#ifndef _SYM
  ASSERT_NO_CUDA_ERROR(cudaDeviceSynchronize());
  ...
  if (!( out[i].x == 0 && out[i].y == i && out[i].y < out[j].y )) {
